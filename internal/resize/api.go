...
// read.
func RunEventHandler(updateInterval time.Duration) { // Blocks, forever
	if updateInterval <= 0 {
...
func InvokeResizeApiStub(ns NamespaceResizeEvent) error {
	...
	fastMergeExample := ResourceQuotaPatch(ns.New)
...
func ResourceQuotaPatch(res resources.Resources) []byte {
	return []byte(fmt.Sprintf("{\"spec\": {\"hard\": {\"cpu\": null, \"memory\": null, \"requests.cpu\": \"%dm\", \"limits.cpu\": \"%dm\", \"requests.memory\": \"%dM\", \"limits.memory\": \"%dM\"}}}",
